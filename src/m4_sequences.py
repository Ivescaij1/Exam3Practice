...
def practice_problem4a(sequence):
    ...
    for k in range(len(sequence) - 1):
        if sequence[k] == sequence[k + 1]:
            indices_to_return = indices_to_return + [k]
...
def practice_problem4b(sequence):
    ...
    ####################################################################
    half_length = round(len(sequence)/2)
    largest_at_even = sequence[0]
    for k in range(half_length):
        if sequence[2 * k] >= largest_at_even:
            largest_at_even = sequence[2 * k]
    return largest_at_even
...
def practice_problem4c(points):
    ...
    for i in range(len(points)):
        x_to_test = points[i].x
        y_to_test = points[i].y

        if is_prime(x_to_test) and is_prime(y_to_test):
            points[i].x = y_to_test
            points[i].y = x_to_test
...
def practice_problem4d(sequence):
    ...
    for k in range(len(sequence) - 1):
        if is_prime(sequence[k]) and is_prime(sequence[k + 1]):
            if sequence[k] != sequence[k + 1]:
                prime_sum = prime_sum + sequence[k]
